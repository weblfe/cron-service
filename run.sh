...
function init() {
    if [ ! -f "./.docker/etc" ];then
      mkdir ./.docker/etc/ -p  && cp ./api/etc/cronService.yaml .docker/etc/cronService.yaml
    fi
...
function docs() {
    ...
    goctl api plugin -plugin goctl-swagger="swagger -filename cronService.json" -api api/cronService.api -dir .
}

function update() {
      goctl api go  -api api/cronService.api -dir api/
}

function apiDocs() {
    docker run --rm -p 8081:8080 -e SWAGGER_JSON=/app/cronService.json -v $PWD:/usr/share/nginx/html/app swaggerapi/swagger-ui
}

function dev() {
    go run api/cronService.go -f api/etc/cronService.yaml
}
